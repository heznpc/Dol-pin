...
export default function Detail({params}: {params: Promise<{id: string}>}) {
  ...
  return <article className="grid gap-12 md:grid-cols-2">
    ...
    <div className="flex flex-col gap-6"><h1 className="text-4xl font-bold">{data.title}</h1><p className="text-2xl font-semibold text-primary">{formatWon(data.daily_price)} / 일</p><p className="text-muted-foreground">보증금 {formatWon(data.deposit)}</p><p className="whitespace-pre-wrap leading-8">{data.description}</p><p>인수·반납 장소: {data.pickup_note??'등록된 장소 정보가 없습니다.'}</p><p className="text-muted-foreground">직접 인수·반납</p><p hidden={data.lender_id!==session?.user.id} className="text-muted-foreground">내가 등록한 물품입니다.</p>{data.lender_id!==session?.user.id?<Button asChild><Link href={`/items/${id}/reserve`}>대여 기간 선택</Link></Button>:null}</div>
  </article>;
}
