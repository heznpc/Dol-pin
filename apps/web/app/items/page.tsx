...
export default function ItemsPage(){return <Suspense fallback={<p>물품을 불러오고 있습니다.</p>}><Items/></Suspense>;}
function Items() {
  const {api} = useApi();
  ...
  const items = useInfiniteQuery({queryKey:['items',concertId,category,search],queryFn:({pageParam})=>api.items({search,category,concertId},pageParam),initialPageParam:undefined as RentalCursor|undefined,getNextPageParam:page=>page.nextCursor});
  ...
  return <div className="flex flex-col gap-8">
    ...
    <div className="grid items-start gap-10 md:grid-cols-[240px_1fr]">
      <aside className="flex flex-col gap-8 rounded-lg bg-card p-5" aria-label="물품 필터">
        <Field><FieldLabel htmlFor="concert">콘서트</FieldLabel><Select value={concertId || 'all'} onValueChange={v => setConcertId(v === 'all' ? '' : v)}><SelectTrigger id="concert" className="w-full"><SelectValue/></SelectTrigger><SelectContent><SelectGroup><SelectItem value="all">전체 콘서트</SelectItem>{concerts.data?.pages.flatMap(page=>page.rows).map(c => <SelectItem key={c.id} value={c.id}>{c.title}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
        ...
        {concerts.hasNextPage?<Button variant="outline" disabled={concerts.isFetching} onClick={()=>{void concerts.fetchNextPage();}}>공연 더 보기</Button>:null}
      </aside>
      <section className="flex min-w-0 flex-col gap-6" aria-label="상품 목록">
        ...
        {items.isPending ? <Skeleton className="h-80 w-full"/> : rows.length ? <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">{rows.map(item => <Link key={item.id} href={`/items/${item.id}`} aria-label={item.title}>
        ...
        {items.hasNextPage?<Button disabled={items.isFetching} onClick={()=>{void items.fetchNextPage();}}>물품 더 보기</Button>:null}
      </section>
    </div>
  </div>;
}
