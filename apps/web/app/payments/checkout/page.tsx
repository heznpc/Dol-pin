...
type Phase='loading'|'ready'|'processing'|'paid'|'expired'|'unavailable';
...
export default function CheckoutPage(){
 ...
 useEffect(()=>{
  ...
  async function poll(){
   ...
   try{const result=await call({action:'status',orderId,token});if(!active)return;setConnectionLost(false);if(apply(result.status))return;}
 ...
 return <section className="mx-auto flex max-w-md flex-col gap-6"><h1 className="text-3xl font-bold">{phase==='paid'?'결제 완료':phase==='processing'?'결제 결과 확인 중':phase==='expired'?'결제 기한이 끝났어요':'예약 결제'}</h1>
 <Failure error={error}/>
 ...
 {phase==='processing'?<div role="status" className="flex flex-col gap-3"><p>중복 결제하지 마세요. 결제 결과를 자동으로 확인하고 있습니다.</p><p>{connectionLost?'연결이 원활하지 않아 다시 확인 중입니다. 결제 실패로 확정된 상태는 아닙니다.':'잠시 기다려 주세요. 확인이 끝나면 이 화면이 자동으로 바뀝니다.'}</p><p className="text-sm text-muted-foreground">화면을 닫아도 괜찮습니다. 거래 상세에서 결과를 이어서 확인할 수 있습니다.</p></div>:null}
