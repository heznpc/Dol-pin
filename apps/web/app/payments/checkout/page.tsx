...
type Checkout = {orderId:string;amount:number;customerKey:string;reservationId:string;mobile:boolean;canPay:boolean;status:string};
...
export default function CheckoutPage() {
 ...
 async function confirm() {
  ...
  try {
   ...
   if(result.status!=='paid') {setError(new Error(result.status==='expired'?'결제가 완료되지 않아 예약이 만료되었습니다.':'승인 결과를 확인 중입니다. 잠시 후 다시 확인해 주세요.'));return;}
