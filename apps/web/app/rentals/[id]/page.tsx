...
export default function Rental({params}:{params:Promise<{id:string}>}){
 ...
 const rental=useQuery({queryKey:['rental',id,session?.user.id],queryFn:()=>api.rental(id),enabled:!!session,refetchInterval:query=>query.state.data?.status&&['requested','accepted','pending','paid','picked_up','returned','disputed'].includes(query.state.data.status)?5000:false});
 ...
 return <section className="mx-auto flex max-w-2xl flex-col gap-8"><h1 className="text-3xl font-bold">거래 상세</h1><Failure error={rental.error??action.error??payment.error??command.error??returnPhoto.error??evidence.error??(hasRecovery?recovery.error:null)}/>
 ...
 <Card><CardHeader><CardDescription>{r.lender_id===session?.user.id?'빌려주는 거래':'빌리는 거래'}</CardDescription><CardTitle><h2>{guidance?.title}</h2></CardTitle></CardHeader><CardContent><p>{guidance?.body}</p>{hasRecovery&&recovery.data?.next_retry_at?<p>다음 확인 예정: {formatKoreaTime(recovery.data.next_retry_at)} 이후</p>:null}{hasRecovery&&recovery.data?.state==='needs_review'?<p className="break-all">문의용 거래 번호: {recovery.data.reference}</p>:null}{r.status==='accepted'&&r.payment_due_at?<p className="mt-3 font-medium">결제 기한 {formatKoreaTime(r.payment_due_at)}</p>:null}</CardContent><CardFooter className="flex-col items-stretch gap-3">
 ...
 {r.status==='returned'&&r.lender_id===session?.user.id?<Button disabled={command.isPending} onClick={()=>command.mutate(r.payment_action?'recover':'settle')}>{r.payment_action?'처리 상태 다시 확인':'반납 수령 확인 · 보증금 반환'}</Button>:null}
