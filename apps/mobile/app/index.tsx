...
export default function Explore() {
  ...
  const items = useQuery({queryKey: ['items', filter.concertId, filter.category, filter.search], queryFn: () => api.items(filter)});
  return <Page><FlatList data={items.data ?? []} keyExtractor={item => item.id} contentContainerStyle={s.content}
    ...
    ListHeaderComponent={<View style={{gap: 20}}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap: 8}}>
        ...
        {concerts.data?.map(c => <Chip key={c.id} label={c.title} selected={filter.concertId === c.id} onPress={() => filter.set({concertId: c.id})}/>)}</ScrollView>
