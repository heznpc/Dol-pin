...
export default function Rentals(){
 ...
 ListHeaderComponent={<><Text style={s.title}>내 거래</Text><View style={{flexDirection:'row',gap:12}}><Button label="진행 중" secondary={!activeOnly} onPress={()=>setActiveOnly(true)}/><Button label="전체 이력" secondary={activeOnly} onPress={()=>setActiveOnly(false)}/></View><ErrorText error={rentals.error} retrying={rentals.isFetching} onRetry={()=>{void rentals.refetch();}}/></>}
 ...
 ListEmptyComponent={rentals.isPending?<ActivityIndicator color={colors.primary}/>:rentals.isError?null:<Text style={s.muted}>아직 거래가 없습니다.</Text>}
