...
모바일 계정 화면은 이메일·비밀번호를 기본 로그인 수단으로 제공한다.
회원가입에는 비밀번호 재입력이 필요하고, 이메일 확인 전에는 계정 사용을 허용하지 않는다.
전화번호 OTP와 소셜 로그인은 추가 수단으로 유지한다.
확인되지 않은 이메일로 로그인하면 확인 메일 재전송을 제공한다.
비밀번호는 컴포넌트 메모리에서만 다루며 가입·로그인 성공 또는 모드 변경 시 지운다.
로그인 세션은 기존 SecureStore를 사용한다.

- `npm run test:email-auth`: 입력값 검증, 기존 비밀번호 호환, 안전한 오류 문구.
- `npm run test:email-auth:local`: 실제 로컬 Auth 가입, 확인 전 로그인 거절,
  Mailpit 확인 메일, PKCE 앱 콜백, 프로필 생성, 틀린 비밀번호, 새 클라이언트 재로그인.
  루프백 Supabase와 로컬 Mailpit만 허용하며 외부 이메일 수신자에게 보내지 않는다.

`supabase/config.toml`은 이메일 확인을 켠다. 호스팅 Auth에도 이메일 가입과
Confirm email을 활성화하고 SMTP를 설정해야 한다. 허용 redirect URL은
`dolpin://auth/callback`이며 확인 링크는 기존 PKCE 콜백으로 처리한다.
참고: [Supabase 이메일 가입](https://supabase.com/docs/reference/javascript/auth-signup),
[확인 메일 재전송](https://supabase.com/docs/reference/javascript/auth-resend).

2026-09-10 Release iPhone Simulator 실측: 이메일 회원가입 → 확인 전 로그인
거절 팝업 → 로컬 확인 메일 링크로 앱 복귀 → 프로필 생성 → 로그아웃 →
틀린 비밀번호 안내 → 정상 재로그인 → 앱 재시작 후 세션 유지까지 확인했다.
기존 전화번호 OTP·프로필·RLS 검사도 통과했다. 시뮬레이터의 하드웨어 한글
입력 모드가 자동 타이핑을 변환하여, 이메일과 비밀번호는 접근성 입력으로 검증했다.
검증 후 테스트 계정은 로그아웃했고 기본 이메일 로그인 화면을 열어 두었다.
