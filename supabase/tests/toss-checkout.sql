...
DO $$ DECLARE c public.toss_checkouts; r uuid := current_setting('test.rental_id')::uuid; BEGIN
 BEGIN
 ...
 PERFORM public.begin_toss_confirmation(c.order_id);
 PERFORM public.finish_toss_confirmation(c.order_id,'test_payment');
 PERFORM public.finish_toss_confirmation(c.order_id,'test_payment');
 ...
 BEGIN
  PERFORM public.finish_toss_confirmation(c.order_id,'different_payment');
 ...
 IF has_function_privilege('authenticated','public.finish_toss_confirmation(text,text)','EXECUTE')
