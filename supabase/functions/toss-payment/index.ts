...
export function createHandler(providers:ProviderFactory=paymentProvider) {
 return async(req:Request):Promise<Response>=>{
  ...
  if(!body || typeof body!=='object')return jsonResponse(400,{error:'잘못된 결제 요청입니다.'});
  ...
  try {
   if(body.action==='prepare' || body.action==='recover') {
    ...
    if(body.action==='recover') {
     const {data:r,error}=await admin.from('reservations').select('borrower_id,status').eq('id',body.reservationId).single();
     if(error || r.borrower_id!==caller.id)return jsonResponse(403,{error:'거래에 접근할 수 없습니다.'});
     if(r.status!=='accepted')return jsonResponse(200,{status:r.status,reservationId:body.reservationId});
     const {data:c}=await admin.from('toss_checkouts').select('order_id').eq('reservation_id',body.reservationId).maybeSingle();
     return jsonResponse(200,c?await reconcileCheckout(admin,c.order_id,undefined,providers):{status:r.status,reservationId:body.reservationId});
    }
    providers('toss'); // Validate provider configuration before opening a checkout.
    const origin=Deno.env.get('DOLPIN_WEB_URL');if(!origin)return jsonResponse(503,{error:'결제 웹 주소 설정이 필요합니다.'});
    const token=crypto.randomUUID()+crypto.randomUUID();
    const c=await rpc<{order_id:string}>(admin,'prepare_toss_checkout',{p_reservation_id:body.reservationId,p_actor:caller.id,p_token_hash:await hash(token),p_mobile:body.mobile===true});
    return jsonResponse(200,{checkoutUrl:`${new URL('/payments/checkout',origin)}#token=${token}&orderId=${c.order_id}`});
   }
   if(!['checkout','confirm','status'].includes(String(body.action)) || typeof body.token!=='string' || typeof body.orderId!=='string')
    return jsonResponse(400,{error:'잘못된 결제 요청입니다.'});
   const {data:session,error:sessionError}=await admin.from('toss_checkout_sessions').select('mobile,expires_at').eq('order_id',body.orderId).eq('token_hash',await hash(body.token)).maybeSingle();
   if(sessionError || !session || Date.parse(session.expires_at)<=Date.now())return jsonResponse(403,{error:'결제 링크가 유효하지 않습니다. 거래 상세에서 결과를 확인해 주세요.'});
  ...
  } catch(e) {return jsonResponse(502,{error:e instanceof Error?e.message:'결제 상태를 확인하지 못했습니다. 거래 상세에서 다시 확인해 주세요.'});}
 };
}
