-- A provider-authenticated identity can lack email/phone consent. Do not
-- fabricate contact data or identity_verified. auth.identities is Auth-owned.
...
BEGIN
  ...
  SELECT CASE WHEN phone_confirmed_at IS NOT NULL THEN nullif(phone,'') END,
    NOT coalesce(is_anonymous,false) AND (
      ...
      (email_confirmed_at IS NOT NULL AND nullif(email,'') IS NOT NULL) OR
      EXISTS (SELECT 1 FROM auth.identities i WHERE i.user_id=actor
        AND i.provider IN ('kakao','custom:naver') AND nullif(i.provider_id,'') IS NOT NULL))
