...
| 영역 | 코드와 실구동으로 확인한 범위 |
| --- | --- |
| 모바일 | React Native + Expo: 전화번호 인증, 프로필, 물품 탐색·등록·사진 업로드·상세 |
...
기존 Flutter 신규 예약 RPC 권한을 제한한 branch이므로, 새 결제 경로 검증
...
Flutter 참고 구현은 SDK 3.44.8을 `mise.toml`에 고정하여 기존 분석·테스트를 유지합니다.
