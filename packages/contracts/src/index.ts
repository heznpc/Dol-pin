...
export function rentalGuidance(r:{status:string|null;payment_action:string|null;payment_attempt_merchant_uid:string|null},lender:boolean) {
 if(r.payment_action)return {title:'환불 결과를 확인하고 있어요',body:'확인이 끝날 때까지 물품 전달·반납을 진행하지 마세요. 이 화면을 닫아도 확인은 계속됩니다.'};
 switch(r.status) {
  ...
  case 'accepted':return r.payment_attempt_merchant_uid?{title:'결제 결과를 확인하고 있어요',body:'중복 결제하지 마세요. 결과를 자동으로 확인하며, 화면을 닫아도 거래 상세에서 이어서 확인할 수 있습니다.'}:lender?{title:'빌리는 분의 결제를 기다리고 있어요',body:'결제 완료를 확인한 뒤 약속한 장소에서 물품을 전달해 주세요.'}:{title:'결제하면 예약이 확정돼요',body:'아래 기한 안에 대여료와 보증금을 결제해 주세요.'};
